...
if (!apiKey) {
    console.warn("GEMINI_API_KEY is not set — AI proposal and image generation will fail.");
}
...
// 5. IMAGE GENERATION — Gemini 3.1 Flash Image
app.post("/api/generate-image", async (req, res) => {
    try {
        ...
        if (base64Output) {
            return res.json({ url: `data:${mimeType};base64,${base64Output}` });
        }

        throw new Error("No image returned by Gemini 3.1 Flash Image");
    } catch (e: any) {
